...
A: NOT by the 300s idle watchdog — `lifecycle_idle_watchdog_exempt` (`src/phase/lifecycle.mbt:541`) returns `true` for `Dev(ReviewOwned)`, `Dev(ChangesRequested)`, `Dev(ExitRequestedDeferred)`, `Dev(WaitingForRedGate)`, i.e. it exempts exactly the thread-owning states (plus the pre-PR red-gate wait). The original "reuse the idle watchdog" plan was wrong: a process-alive, wedged, `ReviewOwned` leaf is skipped by the idle watchdog (exempt) AND by the dead-only reaper (still alive) → permanent thread orphan, gate never opens. A **long, progress-based backstop is therefore MANDATORY in this work, not deferred.** It must (a) be progress-based (no thread/PR-state advance for N hours), not idle-based — that covers both idle-hang and busy-loop; (b) explicitly cover the watchdog-exempt **thread-owning** states by name — the three that carry a `pr_number` + review state: **`Dev(ReviewOwned)`, `Dev(ChangesRequested)`, `Dev(ExitRequestedDeferred)`** (`lifecycle_thread_owning_dev`, `src/phase/lifecycle.mbt:564`). `Dev(WaitingForRedGate)` is EXCLUDED: it is a pre-PR TDD wait (the leaf sent `[RED GATE]` before filing any PR), owns no review threads, carries no `pr_number`, and is correctly left to the red-gate handshake. (`ExitRequestedDeferred` is non-terminal and owns a PR, so an alive-wedged one is reached by neither watchdog nor reaper and wedges the `is_terminal`-keyed gate — it MUST be covered; this corrects the earlier draft that excluded it.) (c) live in a dedicated selection predicate (`lifecycle_thread_owning_dev`, `src/phase/lifecycle.mbt:564`) rather than scatter the state list — the never-wired `progress_watchdog_applies` was deleted as dead code (RA-3); (d) use a deliberately long net (hours, beyond any legitimate review round), never a tight SLA — a net needs no precise tuning, an SLA does, and a tight one reaps honest slow deliberation. Firing it = automated kill-to-take-over, so it inherits the reconnect-fence requirement below.
...
- **F18 (snapshot source):** the gate reads **live registry lifecycle** (authoritative, request-time); the next_action suppression is *cosmetic*, and a one-tick-lagging poller snapshot would have been acceptable for it (mismatch is safe-by-construction in only one direction — TL handed an action the gate then rejects = harmless). **Implementation note (choir-3rp5.5):** as built, the cosmetic suppression (`poll_owner_released_for_threads` → `owner_released_for_threads_from_lifecycle`, `src/server/handler_poll_delivery.mbt`) reads `@phase.read_lifecycle` — the **authoritative KV lifecycle**, the same source the gate keys on — NOT the poller snapshot this finding originally described. That is a safe, strictly-tighter deviation: the cosmetic layer and the boundary gate now agree on the same request-time lifecycle, so the snapshot-vs-gate mismatch is eliminated rather than merely "harmless in one direction." It costs one extra file read per emission tick, acceptable for a cosmetic layer.
...
- **Mandatory progress-backstop** (F1): a long, progress-based watchdog covering the three thread-owning Dev states `ReviewOwned`/`ChangesRequested`/`ExitRequestedDeferred` (`lifecycle_thread_owning_dev`, `lifecycle.mbt:564`) drives a wedged-but-alive leaf terminal so threads cannot orphan.
...
- SCOPE CORRECTION (Re-audit Finding 3): `progress_watchdog_applies` (`src/phase/lifecycle.mbt:554`) has ZERO runtime callers — it is dead code; the live watchdog is `check_idle_agents` (`handler_disconnect.mbt:1065`, driven from `uds_server.mbt:3835`, threshold 300L) keyed on the DIFFERENT predicate `lifecycle_idle_watchdog_exempt`. There is no consumer loop to "extend." Leaf 2 must BUILD a progress-watchdog runtime path: a new long-threshold sweep on the poller fiber keyed on a dedicated selection predicate (`lifecycle_thread_owning_dev`, `src/phase/lifecycle.mbt:564`). Cover the three thread-owning Dev states by name — **`Dev(ReviewOwned)`, `Dev(ChangesRequested)`, `Dev(ExitRequestedDeferred)`** — each carries a `pr_number` + review state and is `lifecycle_idle_watchdog_exempt:541`. EXCLUDE `Dev(WaitingForRedGate)`: it is the pre-PR red-gate wait, owns no review threads, carries no `pr_number`, and cannot orphan contested threads. (The never-wired `progress_watchdog_applies` was deleted as dead code, RA-3.) This is materially more than a predicate edit — decompose it as such.
...
## Verify
- `moon test --target native` — unit/whitebox: `owner_may_write` truth table (TL-blocked, leaf-allowed, fenced-terminal-leaf, no-leaf-TL-allowed, TL-owner-allowed); backstop predicate covers `ReviewOwned`/`ChangesRequested`/`ExitRequestedDeferred` (and excludes `WaitingForRedGate`) and fires only past the long threshold; "PR healthy" classifier over `TrackedPR` field combinations.
