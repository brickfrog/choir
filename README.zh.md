...
Choir 是一个用 MoonBit 编写的本地代理编排器。一个昂贵的模型担任团队负责人（TL，Claude）；更便宜的代理作为叶子（leaf）运行（默认 Codex，也可选 Gemini、Moon Pilot 或 Cursor）。每个叶子在自己的 git worktree 中工作，并向 TL 的分支提交 PR。内置的 poller 自动处理 Copilot review，把 GitHub review 和 CI 反馈路由到对应面板，并在 PR 获批时通知 TL。核心循环是 scaffold、fork、converge：TL 提交共享类型，派生一波并行叶子，逐个合并已批准的 PR，然后再派生下一波或向上提交自己的 PR。
...
flowchart TD
    ...
    tl -. "可选，多波次" .-> spec["/crystallize 然后 /decompose<br/>feature/&lt;slug&gt; 分支"]
    spec -.-> tl
    ...
    la -->|"file_pr"| pr["PR 到父分支"]
    lb -->|"file_pr"| pr
    pr <--> gh[("GitHub：review / CI")]

    subgraph optional["可选：Copilot 自动 review"]
        copilot["Copilot review"]
    end
    gh <-.-> copilot

    gh --> poller["Poller"]
    poller -->|"review / CI 反馈到对应面板"| la
    poller -->|"review / CI 反馈到对应面板"| lb
    poller -->|"已批准 + 快照"| tl
...
MIT 许可证。架构参考了 [exomonad](https://github.com/tidepool-heavy-industries/exomonad)。
