...
bd --readonly show <id> --json  # worker/read-only 流程读取 issue 详情
...
Choir 会把实时编排的关键节点 best-effort 镜像回 Beads：spawn 的认领/元数据、
`file_pr` 的 PR 记录、`notify_parent` 的交接 notes、`report_usage` 的用量遥测、
`merge_pr` 的关闭动作，以及简洁的 audit 记录。实时生命周期、PR、review、CI、
thread、verify 和 merge 判断仍以 Choir 为准；Beads 是持久 backlog 和交接日志。
