...
operation is meant to stay conversational. `choir goal archive` is the one
exception: it has no Conductor tool, because what an installation keeps on disk
is an operator decision rather than a steering one.
...
| `choir goal archive <goal-id\|--part bead-id> [--dry-run]` | Releases one terminal Goal's durable storage: its idempotency witness refs — the Goal-namespaced ones and the per-Part integration witnesses its own Parts recorded — and stored content whose every recorded retention has expired. It keeps the durable Goal record, the Goal's branch, and any content another Goal or a permanent retention still holds. It does not touch sandbox runtime roots or BoxLite boxes — the runtime root goes at the Goal's terminal boundary and on the next daemon start, and the box store is shared by every Goal in the repository. A Goal that is active, paused, blocked, or recovery-uncertain is refused with the reason. `--dry-run` reports exactly what would be released and releases nothing. | **Yes for an apply — it deletes witness refs and expired content. No for `--dry-run`.** |
...
| `choir stop --purge` | Everything `choir stop` does, then `purge_native_goal_resources`: requires exact ownership receipts for every matching repository runtime, reconciles their BoxLite boxes, removes the canonical repository-scoped BoxLite home, reports reclaimed bytes, deletes the exact owned local Goal/witness Git refs, releases Bead claims, and finally removes `.choir/state` and `.choir/run`. It does not delete ambiguous global caches, external version bundles, user branches, remote branches, PRs, or source Beads. Missing or ambiguous ownership and any external cleanup failure keep the database and exit nonzero so the purge can be retried. | **Yes — it discards durable Goal state, the repository BoxLite cache, and owned Goal refs. This is not a routine unblock step.** Use it only when durable state is genuinely unusable and you have accepted losing in-flight Goals. |
